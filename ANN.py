...
if __name__ == "__main__":
    ...
    lstmQ = Dense(200, activation='relu')(inputQ)
    lstmC = Dense(200, activation='relu')(inputC)
    encoder = merge([lstmQ, lstmC], mode='concat')
    attention = Dense(200, activation='relu')(encoder)
    attention = Flatten()(attention)
    attention = Dense(1+maxLength, activation='softmax')(attention)
    ...
    #    model = load_model("cvqaLabelLSTMSoftmaxAttention.h5", custom_objects={'Attention':Attention, 'binary_loss':binary_loss})
    ...
    try:
        for epoch in range(1000):
            ...
            print("Training accuracy:", 100*test(model.predict([X_questions_train, X_captions_train]), y_train)[0])
            testacc, th = test(model.predict([X_questions_test, X_captions_test]), y_test)
            print("Validation accuracy:", 100*testacc)
            if testacc > best:
                model.save("cvqaLabelLSTMSoftmaxMLP.h5")
                threshold = th
                best = testacc
                print("Saved.")
