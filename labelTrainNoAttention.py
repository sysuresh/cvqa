...
lstmQ = Bidirectional(GRU(200, return_sequences=True))(inputQ)
lstmC = Bidirectional(GRU(200, return_sequences=True))(inputC)
encoder = merge([lstmQ, lstmC], mode='concat')
encoder = Flatten()(encoder)
attention = Dense(1+maxLength, activation='softmax')(encoder)
model = Model(input=[inputQ, inputC], output=attention)
model = load_model("NOATTENTION94.h5")
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
print(model.summary())
json.dump(model.to_json(), modelArch)
modelArch.close()
#model = load_model("cvqaLabelLSTM.h5", custom_objects={'Attention':Attention, 'binary_loss':binary_loss})
...
if len(sys.argv) > 1 and sys.argv[1] == "train":
    try:
        ...
        for epoch in range(1000):
            ...
            if testacc > best:
                model.save("cvqaLabelLSTMSoftmaxNoAttention.h5")
                best = testacc
                print("Saved.")
