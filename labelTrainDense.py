...
if __name__ == "__main__":
    ...
    lstmQ = Dense(400, activation='relu')(inputQ)
    lstmC = Dense(400, activation='relu')(inputC)
    encoder = merge([lstmQ, lstmC], mode='concat')

    fc = Flatten()(encoder)
    fc = Dense(5*(1+maxLength), activation='relu')(fc)
    fc = Dense(1+maxLength, activation='sigmoid')(fc)
    model = Model(input=[inputQ, inputC], output=fc)
    #model = load_model("ATTENTION90.h5", custom_objects={'Attention' : Attention})
    model = load_model("DENSE85.h5")
