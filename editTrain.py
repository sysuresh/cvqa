...
def extractFeatures(questionRows, totalLength, maxLength, questionToTag, captionToNounList, actions):
    ...
    for i,questionRow in questionRows.iterrows():
        ...
        if i < (len(questionRows) // 3):
            replacements += [(conj(wNoun, questionToTag[i].split(",")[0].split(",")[0]), conj(wVerb, questionToTag[i].split(",")[1])) for wNoun in captionToNounList[caption] for wVerb in actions]
        elif i <= 2 * len(questionRows) // 3:
            replacements += [conj(w, questionToTag[i].split(",")[1]) for w in captionToNounList[caption]]
        else:
            replacements += [conj(w, questionToTag[i]) for w in actions]
...
X_IrrelevantQuestionsTotal, X_CaptionsTotal, y_LabelTotal, X_QuestionEditsTotal, X_RawQuestionEditsTotal, y_RelevantQuestionsTotal, indices, errors = extractFeatures(dataRows, totalLength, maxLength, questionToTag, captionToNounList, actions)
X_IrrelevantQuestionsTotal, X_IrrelevantQuestions, X_CaptionsTotal, X_Captions, y_LabelTotal, y_Label, X_QuestionEditsTotal, X_QuestionEdits, X_RawQuestionEditsTotal, X_RawQuestionEdits, y_RelevantQuestionsTotal, y_RelevantQuestions = ttsplit(X_IrrelevantQuestionsTotal, X_CaptionsTotal, y_LabelTotal, X_QuestionEditsTotal, X_RawQuestionEditsTotal, y_RelevantQuestionsTotal, test_size=100 / len(X_IrrelevantQuestions), random_state=1)
print(X_QuestionEdits.shape)
print("Errors:", errors)

print("Loading model...")
model = modelAttention = load_model("cvqaLabelLSTMSigmoidAttention.h5", custom_objects={'Attention':Attention})
...
correctEdits = [0 for i in range(15)]
